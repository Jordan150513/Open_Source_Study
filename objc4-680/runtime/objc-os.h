...
//判断是64位系统还是32位系统 确定好不同系统下 字长位数值
#ifdef __LP64__
...
//什么是字节对齐？什么时候需要字节对齐？
//捡起计算机底层的知识：
//什么时候都需要字节对齐，cpu硬件或者外部总线获取数据的时候是按照块（一块可能是8byte 可能是16byte）获取，不是1byte单位获取，对齐之后存取使用更方便
...
//内联函数 避免了调用函数的时候 保护现场 恢复现场的开销

// Mix-in for classes that must not be copied.
//对象不能被拷贝的类，？？因为拷贝构造函数和 = 运算符都被删除了（不生成默认的版本）
//函数被删除之后，重载函数也是非法的---不太理解？？
class nocopy_t {
  private:
...
//mach-o/dyld_priv.h 文件 not found 需要手工导入
